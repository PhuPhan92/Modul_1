...
public class MSOrderService extends DBContext implements IOrderService {
    private static final String ADD_ORDER_ITEM = "INSERT INTO `demo_case_modul3`.`order_item` (`id_order`, `id_product`, `quantity`) VALUES (?, ?, ?)";
    String ADD_ORDER = "INSERT INTO `demo_case_modul3`.`order` (`address`, `phone`, `name`, `total`) VALUES (?, ?, ?, ?)";
    @Override
...
//         Thực thi gọi procedure không có tham số
    ...
    public void saveOrderBySPWithParameter(Order order) {
        ...
        System.out.println(jsonInString);
        try {
            CallableStatement callableStatement = connection.prepareCall("call demo_case_modul3.spSaveOrderWithParameter(?, ?)");
            callableStatement.setObject(1, jsonInString);
