<template>
  <div id="mainW" :class="typeof weather != 'undefined' && weather.temp_c > 16 ? 'warm' : ''">
    <main>
      ...
      <div v-if="hasHistory">
        <p class="history-title"> Search History</p>
        <div >
          <HistoryItem v-for="(history,index) in history" :key="index" :history="history"></HistoryItem>
        </div>
...
export default {
  ...
  data () {
    return{
      query: "",
      api_key: 'ed7f4635de504a83be233222231108',
  ...
  computed: {
    ...mapGetters(['weather', 'location','forecastday' ]),
  },
  methods: {
    fetchWeather(e) {
      if (e.key == "Enter") {
        
        fetch(`${this.url_base}forecast.json?key=${this.api_key}&q=${this.query}&days=3&aqi=no&alerts=no`)
          .then(res => {
            res.json().then((data) => {
              this.check = true;
              this.$store.dispatch('setWeather', data.current);
